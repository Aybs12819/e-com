...
export async function POST(req: Request) {
  try {
    ...
    if (!product || product.length === 0) {
      return NextResponse.json(
        { error: "Failed to retrieve product after insertion." },
        { status: 500 }
      );
    }
    ...
    if (variants && variants.length > 0) {
      ...
      const variantData = variants.flatMap((variant: any) =>
        variant.value
          ? variant.value.map((val: string, idx: number) => ({
              product_id,
              type: variant.type,
...
export async function DELETE(req: Request) {
  try {
    ...
    if (deleteVariationsError) {
      console.error(
        "Error deleting product variations:",
        deleteVariationsError
      );
      return NextResponse.json(
        { error: deleteVariationsError.message },
        { status: 500 }
      );
    }
