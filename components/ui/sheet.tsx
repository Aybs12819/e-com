...
const sheetVariants = cva(
  "fixed inset-0 z-50 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-300 data-[state=open]:duration-500",
  {
    variants: {
      side: {
...
>(({ side = "right", className, children, ...props }, ref) => (
  <SheetPrimitive.Portal>
    ...
    <SheetPrimitive.Content
      ref={ref}
      className={cn(sheetVariants({ side }), "bg-background grid h-full gap-4 p-6 shadow-lg", className)}
      {...props}
    >
      {children}
